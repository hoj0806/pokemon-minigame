...
const DIFFICULTY_OPTIONS: DifficultyOption[] = [
  {
    id: 'easy',
    label: '쉬움',
    colorClass: 'border-game-success text-game-success',
  },
  {
    id: 'normal',
    label: '보통',
    colorClass: 'border-game-warning text-game-warning',
  },
  {
    id: 'hard',
    label: '어려움',
    colorClass: 'border-game-error text-game-error',
  },
];
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-57px)] gap-8 p-6">
        ...
        <div className="flex flex-col gap-4 w-full max-w-xs">
          {DIFFICULTY_OPTIONS.map(({ id, label, colorClass }) => (
            <button
              key={id}
              onClick={() => handleDifficultySelect(id)}
              className={`rounded-[--radius-card] border-2 bg-[--color-surface-raised] shadow-[--shadow-card] px-6 py-5 text-left cursor-pointer hover:shadow-[--shadow-elevated] transition-shadow ${colorClass}`}
            >
    ...
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-57px)] gap-8 p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full max-w-2xl">
        {GAMES.map(({ id, title, description }) => (
          ...
          >
            <h2 className="font-galmuri text-xl font-bold text-[--color-on-surface] mb-2">
              {title}
            </h2>
